...
function update(dt)
    ...
    if self.upgradeState.upgrading == true then
        ...
        if os.time() >= self.upgradeState.endTime then
            ...
            applyBlackUpgradeAppearance(newItem)
    ...
    if not self.itemStorage[1] then
        ...
        if not self.hasValidItem then
            ...
            for _, armorType in pairs({"head", "chest", "legs"}) do
                for _, v in pairs(self.config.validItems[armorType] or {}) do
...
function toggleInterface(item)
    if not item then
        ...
        for _, armorType in pairs({"head", "chest", "legs"}) do
            for _, v2 in pairs(self.config.validItems[armorType] or {}) do
                if string.find(item.name, v2) then
                    calculateUpgrade(item, armorType)
                end
            end
        end
    end
...
function calculateUpgrade(item, armorType)
    ...
    for _, v in pairs(self.config.upgradeCosts) do
        if (v.level >= itemLvl) and
           ((v.armorType and v.armorType == "any") or (v.armorType == armorType)
            or (v.weaponType and (v.weaponType == "any" or v.weaponType == armorType))) then
...
function applyBlackUpgradeAppearance(item)
    local itemConfig = root.itemConfig({name = item.name})
    local baseName = item.parameters.shortdescription
        or (itemConfig and itemConfig.config.shortdescription)
        or item.name

    item.parameters.shortdescription = baseName .. "^#1c1c1c; "
    item.parameters.directives = "?replace;ffca8a=969696;e0975c=6a6a6a;a85636=3b3b3b;6f2919=1c1c1c"
end
