...
function init()
  ...
  self.minTriggerDamage = 0 --Minimum damage you have to do in one hit in order to trigger
  self.help = 0 --This is to keep track of wether or not the NPC has called the Space Police
  self.random = math.random(1,1) --Chance to call Police, because balance I guess. Every call gets 2 helpers so 1/5 is okay, right???
  self.randomhealth = math.random(25,75)/100 --At what health percentage the NPC calls for help. Some mistakes should be forgiven, right? Some randomness for fun.
...
function update(dt)
	...
	local damageNotifications, nextStep = status.damageTakenSince(self.queryDamageSince) --I might've put this whole thing in the wrong place, but eh, whatever, it works.
    self.queryDamageSince = nextStep
    for _, notification in ipairs(damageNotifications) do
      if notification.healthLost > self.minTriggerDamage and status.resourcePercentage("health") < self.randomhealth and self.help == 0 and notification.sourceEntityId ~= notification.targetEntityId and self.random == 1 then --Checks if the player has damaged the NPC and stuff.
		if math.random(1,4) == 1 then --1 in 4 chance of spawning Coptop team.
	  	world.spawnNpc({mcontroller.xPosition() + 2, mcontroller.yPosition()}, "shadow", "shadowguard", world.threatLevel()) --Player attacked the NPC, call the freakin' Space Police!!!
		world.spawnNpc({mcontroller.xPosition() - 2, mcontroller.yPosition()}, "shadow", "shadowguard", world.threatLevel()) --Y'know what, call 2 of them!
		else
		world.spawnNpc({mcontroller.xPosition() + 2, mcontroller.yPosition()}, "shadow", "shadowguard", world.threatLevel())
		world.spawnNpc({mcontroller.xPosition() - 2, mcontroller.yPosition()}, "shadow", "shadowguard", world.threatLevel())
		end
		self.help = 1 --NPC has called for Space Police, so no more help will come for that specific NPC
        break
      end
	end
...
--This is commented so you can't get away with murder by stun-locking the poor NPC so hard they can't utter a word and can't call for help, you monster.
...
function die()
  --world.spawnMonster("po", mcontroller.position(), { level = 5 })
  --world.spawnNpc(mcontroller.position(), "human", "guard", 4)
  self.quest:die()
  recruitable.die()
  tenant.backup()
  spawnDrops()
